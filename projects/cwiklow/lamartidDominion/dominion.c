...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  int drawntreasure=0;
  // int cardDrawn; // doesn't need to be declared, now that cardDrawn is declared
  // within adventurer_play
  ...
  switch( card )
    {
    case adventurer:
      adventurer_play(currentPlayer, drawntreasure, z, temphand,
        state, handPos, bonus);
      return 0;
    case council_room:
    ...
    case mine:
      return mine_play(currentPlayer, state, handPos, choice1, choice2);
    ...
    case smithy:
      ...
      smithy_play(currentPlayer, state, handPos, bonus);
      return 0;
    ...
    case steward:
      steward_play(currentPlayer, state, handPos, choice1, choice2, choice3);
      return 0;
...
/*******************************************************************************
 * Card-specific calling functions
*******************************************************************************/
void smithy_play(
  ...
  // FIXED!! Corrected trash flag back to 0
  discardCard(handPos, currentPlayer, state, 0);
}
...
void adventurer_play(
  ...
  // Declaare cardDrawn inside this function, rather than cardEffect()
  int cardDrawn;
  ...
  while(drawntreasure < 2){
    ...
    // FIXED! Corrected conditional check back to 1
    if (state->deckCount[currentPlayer] < 1){
      shuffle(currentPlayer, state);
    }
    drawCard(currentPlayer, state);
    // Top card of hand is most recently drawn
    cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer]-1];
    if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
      drawntreasure++;
    else{
      temphand[z] = cardDrawn;
      // Remove top card (most recently drawn)
      state->handCount[currentPlayer]--;
      z++;
    }
  }
...
int mine_play(
  ...
  int store_card = state->hand[currentPlayer][choice1];
  ...
  discardCard(handPos, currentPlayer, state, 0);
  // discard trashed card
  for (int idx = 0; idx < state->handCount[currentPlayer]; idx++){
    if (state->hand[currentPlayer][idx] == store_card){
      discardCard(idx, currentPlayer, state, 0);
      break;
    }
  }
  return 0;

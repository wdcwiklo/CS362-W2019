...
//checks if Great Hall card works correctly
//p: player number
//post: gameState to be used 
//handPos: position of card in deck
//Returns 1 for success, 0 for failure
int checkGreatHall(int p, struct gameState* post, int handPos)
{
	...
	num = cardEffect(great_hall, choice1, choice2, choice3, post, handPos, &bonus);

	//Great Hall should draw one card, provide 1 additional action, and then be discarded
	...
	//check if 1 additional action have been added
	num = pre.numActions;

	if (num + 1 == post->numActions)
	...
	{
		if (post->hand[p][i] == great_hall)
		{
			printf("Discard failed: Great Hall still in hand\n");
			result = 0;
			flag = 0;
		}
	}
...
int main() {
	...
	//put a Great Hall as first card in hand
	G.hand[0][0] = great_hall;

	printf("Test 1:\n");
	testCount++;
	//play Great Hall card
	r = checkGreatHall(0, &G, 0);
	testPassed += r;
	
	//put Great Hall as last card in hand
	G.hand[0][G.handCount[0]] = great_hall;

	printf("Test 2:\n");
	testCount++;
	//play Great Hall card
	r = checkGreatHall(0, &G, G.handCount[0]-1);
	testPassed += r;

	printf("\nCard Test 4 Complete: Passed %d out of %d tests\n\n", testPassed, testCount);
